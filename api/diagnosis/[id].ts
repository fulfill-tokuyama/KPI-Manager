...
async function handlePut(id: string, req: VercelRequest, res: VercelResponse) {
  ...
  try {
    const { sheets, spreadsheetId } = client;
    const response = await sheets.spreadsheets.values.get({ spreadsheetId, range: 'diagnosis!A:A' });
    ...
    const getRow = await sheets.spreadsheets.values.get({ spreadsheetId, range: `diagnosis!A${rowIndex + 1}:P${rowIndex + 1}` });
    ...
    await sheets.spreadsheets.values.update({
      spreadsheetId, range: `diagnosis!A${rowIndex + 1}:P${rowIndex + 1}`,
...
async function handleDelete(id: string, res: VercelResponse) {
  ...
  try {
    ...
    await sheets.spreadsheets.values.clear({ spreadsheetId, range: `diagnosis!A${rowIndex + 1}:P${rowIndex + 1}` });
